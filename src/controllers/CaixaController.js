const { CaixaDia, CaixaLancamento, CaixaCategoria, CaixaFormaTipo, CaixaFormaLanc, Usuario } = require('../models')
...
class CaixaController {
    ...
    async verificarCaixaAberto(req, res) {
        ...
        try {
            const dados = await CaixaDia.findOne({
                ...
                include: [
                    {
                        model: CaixaLancamento, as: "lancamentos", attributes: ['descricao', 'id', 'valor', 'created_at'],
                        include: [
    ...
    async listarCategoria(req, res) {
        ...
        try {
            const dados = await CaixaCategoria.findAll({ where: { ...filtro }, order: ['descricao'] })
            return res.status(200).json({ falha: false, dados: dados })
        } catch (error) {
            console.log(error)
    ...
    async listarCategoriaEntrada(req, res) {
        try {
            const dados = await CaixaCategoria.findAll({ where: { tipo: "Entrada" }, order: ['descricao'] })
            return res.status(200).json({ falha: false, dados: dados })
        } catch (error) {
            console.log(error)
    ...
    async listarCategoriaSaida(req, res) {
        try {
            const dados = await CaixaCategoria.findAll({ where: { tipo: "Saída" }, order: ['descricao'] })
            return res.status(200).json({ falha: false, dados: dados })
        } catch (error) {
            console.log(error)
